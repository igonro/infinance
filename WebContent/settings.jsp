...
<body>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<!-- SIDEBAR -->
			<main role="main" class="col-md-3 pl-sm-auto col-lg-8 pt-3"> <!-- SETTINGS FORMS -->
			<form class="form-register " action="/infinance/settings"
				method="post">
				<h2>Perfil</h2>
				<label for="inputName" class="control-label">Nombre</label>
				<input id="inputName" class="form-control" placeholder=""
					required="true" autofocus="" type="text" name="name"
					value="${userinfo.firstName}">
				<label for="inputLastName" class="control-label">Apellidos</label>
				<input id="inputLastName" class="form-control" placeholder=""
					required="true" autofocus="" type="text" name="lastName"
					value="${userinfo.lastName}">
				<label for="inputPhoneNumber" class="control-label">Teléfono</label>
				<input id="inputPhoneNumber" class="form-control" placeholder=""
					required="true" autofocus="" type="text" name="phoneNumber"
					value="${userinfo.phone}">
				<label for="inputUser" class="control-label">Usuario</label>
				<input id="inputUser" class="form-control" placeholder=""
					required="true" autofocus="" type="text" name="user"
					value="${userinfo.userName}">
				<label for="inputEmail" class="control-label">Correo
					electrónico</label>
				<input id="inputEmail" class="form-control" placeholder=""
					required="true" autofocus="" type="email" name="email"
					value="${userinfo.email}">
				<button id="registerButton" class="btn btn-lg btn-primary btn-block"
					type="submit">Guardar</button>
				<button id="ButtonDialog" class="btn btn-lg btn-primary btn-block"
	...
	<div id="dialog1" title="Cambiar contraseña">
		<div class="container">
			<form id="popupform" action="/infinance/changepassword" method="get">
				<label for="inputPassword" class="control-label">Contraseña
					actual</label>
				<input id="inputPassword" class="form-control"
					placeholder="Contraseña actual" required="true" type="password"
					name="password">
				<label for="inputNewPassword" class="control-label">Nueva
					contraseña</label>
				<input id="inputNewPassword" class="form-control"
					placeholder="Nueva contraseña" required="true" type="password"
					name="newpassword">
				<label for="inputCheckPassword" class="control-label">Repita
					la nueva contraseña</label>
				<input id="inputCheckPassword" class="form-control"
					placeholder="Nueva contraseña" required="true" type="password"
					name="checkPassword">
				...
				<button id="ChangePassword" class="btn btn-lg btn-primary btn-block"
					type="submit" onclick="return Validate()">Cambiar</button>
			</form>
		</div>
	</div>
	...
	<div id="dialogOperation" title="Aviso"></div>
	<script>
		$jq2('#inputCheckPassword').on(
				'keyup',
				function() {
					if ($jq2(this).val() == $jq2('#inputNewPassword').val()) {
						$jq2('#errormessage').html(
								'Contraseñas coinciden &#10004;').css('color',
								'green');
					} else
						$jq2('#errormessage').html(
								'Contraseñas no coinciden &#10008;').css(
								'color', 'red');
				});
	</script>
